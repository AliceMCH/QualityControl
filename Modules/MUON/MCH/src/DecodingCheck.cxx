...
Quality DecodingCheck::check(std::map<std::string, std::shared_ptr<MonitorObject>>* moMap)
{
  ...
  std::cout << "[DecodingCheck::check()] moMap->size(): " << moMap->size() << std::endl;

  for (auto& [moName, mo] : *moMap) {

    std::cout << "[DecodingCheck::check()] moName: " << mo->getName() << "  mGoodFracHistName: " << mGoodFracHistName << std::endl;
    ...
    if (matchHistName(mo->getName(), mGoodFracHistName)) {
      ...
      std::cout << "[DecodingCheck::check()] checking " << mo->getName() << std::endl;
      ...
      for (auto de : o2::mch::constants::deIdsForAllMCH) {
        ...
        if (val < minGoodErrorFrac) {
          ...
          //std::cout << "[DecodingCheck::check()] deId " << deId << " quality Bad" << std::endl;
        } else {
          errorsQuality[deId] = Quality::Good;
          //std::cout << "[DecodingCheck::check()] deId " << deId << " quality Good" << std::endl;
        }
      }
