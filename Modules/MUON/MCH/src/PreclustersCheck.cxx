...
{
  ILOG(Debug, Devel) << "PreclustersCheck::beautify() processing object " << mo->getName() << ENDM;
  ...
  if ((mo->getName().find("RefComp/Efficiency") != std::string::npos)) {
    TCanvas* canvas = dynamic_cast<TCanvas*>(mo->getObject());
    if (!canvas) {
      return;
    }
    ...
    ratioPlot->SetMinimum(1.0 - mEfficiencyRatioScaleRange);
    ratioPlot->SetMaximum(1.0 + mEfficiencyRatioScaleRange);
    ratioPlot->GetXaxis()->SetTickLength(0);
    ...
    if (mo->getName().find("MeanEfficiencyPerSolar") != std::string::npos) {
      ...
      for (int ch = 1; ch <= 9; ch++) {
        firstDE += getNumSolarPerChamber(ch);
        float xpos = firstDE;
        TLine* delimiter = new TLine(xpos, ratioPlot->GetMinimum(), xpos, ratioPlot->GetMaximum());
        delimiter->SetLineColor(kBlack);
        delimiter->SetLineStyle(kDashed);
        ratioPlot->GetListOfFunctions()->Add(delimiter);
      }*/

      if (refCompPlots.first) {
        refCompPlots.first->SetMinimum(mPseudoeffPlotScaleMin);
        ...
        for (int ch = 1; ch <= 9; ch++) {
          firstDE += getNumSolarPerChamber(ch);
          float xpos = firstDE;
          TLine* delimiter = new TLine(xpos, refCompPlots.first->GetMinimum(), xpos, refCompPlots.first->GetMaximum());
          delimiter->SetLineColor(kBlack);
          delimiter->SetLineStyle(kDashed);
          refCompPlots.first->GetListOfFunctions()->Add(delimiter);
        }*/
      }
    } else {
      addChamberDelimiters(ratioPlot);
      ...
      int nDE = getNumDE();
      for (int ch = 1; ch <= 9; ch++) {
        firstDE += getNumDEinChamber(ch - 1);
        float xpos = firstDE;
        TLine* delimiter = new TLine(xpos, ratioPlot->GetMinimum(), xpos, ratioPlot->GetMaximum());
        delimiter->SetLineColor(kBlack);
        delimiter->SetLineStyle(kDashed);
    ...
    if (matchHistName(mo->getName(), mMeanEffRefCompHistNameB) ||
        matchHistName(mo->getName(), mMeanEffRefCompHistNameNB) ||
        matchHistName(mo->getName(), mMeanEffPerSolarRefCompHistName)) {
      drawThreshold(ratioPlot, mMinEfficiencyRatio);
      if (refCompPlots.second) {
        if (checkResult == Quality::Good) {
          refCompPlots.second->SetLineColor(kGreen + 2);
        } else if (checkResult == Quality::Bad) {
